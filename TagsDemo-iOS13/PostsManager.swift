...
let PostImagesCache13 = NSCache<NSString, UIImage>()

//Quickly mocking database using .plist. Not thread-safe.
extension UserDefaults {
	fileprivate static let storedPostsKey = "storedPosts13"
...
//		UserDefaults.standard.storedPosts = []
	...
	private static let queue: DispatchQueue = DispatchQueue(label: "posts-manager.background-queue-13", qos: .background)
	...
	func createPost(_ newPostInput: Post.NewPostInput, completion: @escaping (Result<[Post], PostManagerError>) -> Void) {
		...
		if let img = newPostInput.image {
			...
			if PostImagesCache13.object(forKey: string as NSString) == nil {
				PostImagesCache13.setObject(img, forKey: string as NSString)
			}
		}
		Self.queue.async {
