...
class PostViewModel: ObservableObject {
	...
	init(post: Post) {
		...
		}else if let img = PostImagesCache13.object(forKey: (post.imageURL?.absoluteString ?? post.id) as NSString) {
			self.image = Image(uiImage: img)
		}else {
		...
		PostsManager.shared.randomImage(completion: { [weak self] res in
			...
			case .success(let (img, url)):
				DispatchQueue.main.async {
					...
					self.post.updatePost(image: img, imageURL: url)
					self.image = Image(uiImage: img)
					PostImagesCache13.setObject(img, forKey: (url?.absoluteString ?? self.post.id) as NSString)
				}
